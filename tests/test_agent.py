...
import sys
import shutil
import logging # Added for caplog if needed elsewhere
from unittest.mock import MagicMock, patch, call, ANY # Keep mock imports

# Re-adding sys.path manipulation for this file
src_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '../src')) # Path adjusted from tests/ to src/
...
try:
    ...
    pytest.skip(f"Skipping agent tests due to import error: {e}", allow_module_level=True)
...
def test_learn_step(agent, default_config, mocker):
    ...
    mock_batch = (mock_market, mock_account, mock_action, mock_reward, mock_next_market, mock_next_account, mock_done)
    ...
    mocker.patch.object(agent.buffer, 'sample', return_value=(mock_batch, mock_indices, mock_weights))
    mocker.patch.object(agent.buffer, 'update_priorities') # Mock priority updates
    mocker.patch.object(agent, '_update_target_network') # Mock target updates to isolate learn logic
...
def test_save_load_model(agent, default_config):
    ...
    for key in original_state_dict:
        assert torch.equal(original_state_dict[key], loaded_state_dict[key]), f"Network parameter mismatch for key: {key}"
    ...
    for key in original_state_dict: # Target should match original online after load->sync
        assert torch.equal(original_state_dict[key], loaded_target_state_dict[key]), f"Target network parameter mismatch for key: {key}"
