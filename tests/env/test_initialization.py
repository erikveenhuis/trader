...
class TestTradingEnvInitializationErrors:
    ...
    def test_missing_column(self):
        ...
        with pytest.raises(AssertionError, match=r"Input data missing.*required columns"):
            TradingEnv(
                data_path=mock_path,
                window_size=self.window_size,
                initial_balance=self.initial_balance,
    ...
    def test_insufficient_rows(self):
        ...
        with pytest.raises(AssertionError, match=r"Data length \(\d+\) must be >= window_size \(\d+\)."):
            TradingEnv(
                data_path=mock_path,
                window_size=self.window_size,
                initial_balance=self.initial_balance,
    ...
    def test_invalid_window_size(self):
        ...
        mock_path = create_mock_csv({'open': [1]*10, 'high': [1]*10, 'low': [1]*10, 'close': [1]*10, 'volume': [1]*10}, self.temp_dir.name)
        with pytest.raises(AssertionError, match=r"window_size must be an integer >= 1"):
    ...
    def test_invalid_initial_balance(self):
        ...
        mock_path = create_mock_csv({'open': [1]*10, 'high': [1]*10, 'low': [1]*10, 'close': [1]*10, 'volume': [1]*10}, self.temp_dir.name)
        with pytest.raises(AssertionError, match=r"initial_balance must be a non-negative number"):
    ...
    def test_invalid_transaction_fee(self):
        ...
        mock_path = create_mock_csv({'open': [1]*10, 'high': [1]*10, 'low': [1]*10, 'close': [1]*10, 'volume': [1]*10}, self.temp_dir.name)
        with pytest.raises(AssertionError, match=r"transaction_fee must be a number between 0.*and 1"):
            TradingEnv(data_path=mock_path, window_size=5, transaction_fee=-0.001)
        with pytest.raises(AssertionError, match=r"transaction_fee must be a number between 0.*and 1"):
            TradingEnv(data_path=mock_path, window_size=5, transaction_fee=1.0) # Fee must be < 1 