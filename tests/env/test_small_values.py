...
class TestTradingEnvSmallValues:
    ...
    def setup_method(self, method):
        ...
        data_dict = {
            ...
            'close': [self.small_price_base * (1.0 + 0.1*i/(num_rows-1)) for i in range(num_rows)], 
            'volume': [10] * num_rows
        }
    ...
    def test_buy_with_small_price(self):
        ...
        current_price_in_step = self.small_price_base * (1.0 + 0.1*original_index_for_step/(len(obs_reset['market_data'])+self.window_size-1)) # Approximation based on data gen
        current_price_in_step = self.env.original_close_prices[self.env._map_to_original_index(self.env.current_step)]
        ...
        assert abs(self.env.balance - expected_balance) < 1e-9 # Balance should be near zero
        assert expected_position > 1e-9 # Ensure expected position is not zero before relative check
        assert abs(self.env.position - expected_position) / expected_position < 1e-7 # Position should match calc
        assert abs(info['transaction_cost'] - transaction_cost) < 1e-9 # Cumulative cost
    ...
    def test_sell_with_small_price(self):
        ...
        assert expected_balance > 1e-9 # Ensure expected balance is not zero before relative check
        assert abs(self.env.balance - expected_balance) / expected_balance < 1e-7 
        assert expected_total_cost > 1e-9 # Ensure expected cost is not zero before relative check
        assert abs(info['transaction_cost'] - expected_total_cost) / expected_total_cost < 1e-7
