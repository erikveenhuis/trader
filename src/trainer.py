...
class RainbowTrainerModule:
    def __init__(self, agent: RainbowDQNAgent, device: torch.device, data_manager: DataManager, config: dict):
        ...
        self.best_model_path_prefix = str(Path(self.run_config.get('model_dir', 'models')) / 'rainbow_transformer_best')
        ...
        self.best_trainer_checkpoint_path = str(Path(self.run_config.get('model_dir', 'models')) / 'checkpoint_trainer_best.pt')
        self.validation_metrics = []
        self.performance_tracker = PerformanceTracker()
        self.early_stopping_patience = self.trainer_config.get('early_stopping_patience', 10)
        self.early_stopping_counter = 0
        self.min_validation_threshold = self.trainer_config.get('min_validation_threshold', 0.0)
    ...
    def _save_trainer_checkpoint(self, episode: int, total_steps: int, is_best: bool):
        ...
        try:
            ...
            logger.info(f"Latest checkpoint saved to {self.latest_trainer_checkpoint_path}")
        ...
        if is_best:
            try:
                ...
                logger.info(f"Best checkpoint saved to {self.best_trainer_checkpoint_path}")
                logger.info(f"  New Best Validation Score: {self.best_validation_metric:.4f}")
                logger.info(f"  Episode: {episode}")
                logger.info(f"  Total Steps: {total_steps}")
            except Exception as e:
                logger.error(f"Error saving best checkpoint: {e}")

    def train(self, env: TradingEnv, num_episodes: int,
              start_episode: int, start_total_steps: int,
        ...
        assert isinstance(specific_file, (str, type(None))), "specific_file must be a string or None"
        ...
        logger.info(f"====== STARTING/RESUMING RAINBOW DQN TRAINING ======")
        ...
        logger.info(f"Episodes: {num_episodes}, Update Freq: {self.update_freq}, Log Freq: {self.log_freq}")
        logger.info(f"Agent Gamma: {self.agent.gamma}, LR: {self.agent.lr}, Batch Size: {self.agent.batch_size}")
        logger.info(f"Agent Target Update Freq: {self.agent.target_update_freq}")
        logger.info(f"Agent Atoms: {self.agent.num_atoms}, Vmin: {self.agent.v_min}, Vmax: {self.agent.v_max}")
        logger.info(f"Agent N-Steps: {self.agent.n_steps}")
        logger.info(f"PER Alpha: {self.agent.buffer.alpha}, Beta Start: {self.agent.buffer.beta_start}")
        ...
        for episode in range(start_episode, num_episodes):
            ...
            try:
                 ...
                 assert isinstance(obs['market_data'], np.ndarray), "Market data is not a numpy array"
            ...
            while not done:
                ...
                assert obs['account_state'].shape == (2,), \
                ...
                if total_train_steps < self.warmup_steps:
                     ...
                     assert env.action_space.contains(action), "Sampled warmup action is invalid"
                else:
                     action = self.agent.select_action(obs)
                     assert isinstance(action, (int, np.integer)), "Agent action is not an integer"
                ...
                try:
                    ...
                    assert isinstance(next_obs, dict), "Next observation from env.step() must be a dict"
                    assert 'market_data' in next_obs and 'account_state' in next_obs, "Next observation missing keys"
                    assert isinstance(next_obs['market_data'], np.ndarray), "Next market data is not a numpy array"
                    assert isinstance(next_obs['account_state'], np.ndarray), "Next account state is not a numpy array"
                    assert isinstance(reward, (float, np.float32, np.float64)), "Reward is not a float"
                    assert isinstance(done, (bool, np.bool_)), "Done flag is not boolean"
                ...
                if len(self.agent.buffer) >= self.agent.batch_size and total_train_steps > self.warmup_steps and total_train_steps % self.update_freq == 0:
                    try:
                        ...
                        if loss_value is not None:
                            assert isinstance(loss_value, float), "Loss value is not a float"
                            assert not np.isnan(loss_value) and not np.isinf(loss_value), "Loss value is NaN or Inf"
                ...
                if steps % self.log_freq == 0:
                    ...
                    logger.info(f"  Ep {episode+1} Step {steps}: Port=${metrics['portfolio_value']:.2f}, "
            ...
            avg_loss = episode_loss / (steps / self.update_freq) if steps > 0 and episode_loss != 0 else 0

            metrics = self.performance_tracker.get_metrics()
            logger.info(f"Ep {episode+1}: Reward={episode_reward:.2f}, Mean-{current_window_size}={recent_avg_reward:.2f}, "
                        f"Loss={avg_loss:.4f}, "
            ...
            if val_files and self.should_validate(episode, self.performance_tracker.get_recent_metrics()):
                ...
                logger.info(f"  Difference: {(validation_score - self.best_validation_metric):.4f}")
                ...
                if validation_score > self.best_validation_metric:
                    ...
                    logger.info(f"  Improvement: {(validation_score - self.best_validation_metric):.4f}")
                    logger.info(f"  Saved to: {self.best_model_path_prefix}*")
                else:
                    logger.info(f"  No improvement over previous best model")
                
                # Save checkpoint AFTER validation, potentially marking it as best
                self._save_trainer_checkpoint(episode=episode + 1, total_steps=total_train_steps, is_best=is_best)
            ...
            if save_now:
                 self._save_trainer_checkpoint(episode=episode + 1, total_steps=total_train_steps, is_best=False)
            ...
            save_interval_fallback = 20 # Save less frequently based on training rewards
            if not val_files and episode > min(50, num_episodes // 5) and (episode + 1) % save_interval_fallback == 0:
                 if avg_reward > best_train_reward:
                      best_train_reward = avg_reward
                      self.agent.save_model(self.best_model_path_prefix) # Use same prefix
                      logger.info(f"  SAVED BEST MODEL based on training reward: {self.best_model_path_prefix}* with avg reward: {best_train_reward:.2f}")
        ...
        self._save_trainer_checkpoint(episode=num_episodes, total_steps=total_train_steps, is_best=False) 
        # ------------------

        logger.info(f"====== RAINBOW DQN TRAINING COMPLETED ======")
        ...
        if val_files and self.validation_metrics:
            ...
            logger.info(f"Best validation metric: {best_run['metric']:.4f} (at episode {best_run['episode']})")
            logger.info(f"Best validation model saved to: {self.best_model_path_prefix}*")
        elif not val_files:
    ...
    def evaluate_for_validation(self, env: TradingEnv) -> Tuple[float, dict]:
        ...
        assert isinstance(env, TradingEnv), "env must be an instance of TradingEnv for validation"
    ...
    def validate(self, val_files: List[Path]) -> Tuple[bool, float]:
        ...
        if not val_files:
            logger.warning("validate() called with empty val_files list. Returning default score -inf.")
        ...
        try: # Main validation logic wrapped in try
            ...
            for i, val_file in enumerate(val_files):
                logger.info(f"--- VALIDATING ON FILE {i+1}/{len(val_files)}: {val_file.name} ---")
                try:
                    env = TradingEnv(
                    ...
                    logger.info(f"    Portfolio Value: ${file_metrics['portfolio_value']:.2f}")
                    logger.info(f"    Total Return: {file_metrics['total_return']:.2f}%")
                    ...
                    logger.info(f"    Transaction Costs: ${file_metrics['transaction_costs']:.2f}")
            ...
            avg_metrics = {
                ...
                'portfolio_value': float(np.mean([m['portfolio_value'] for m in metrics])),
                ...
                'transaction_costs': float(np.mean([m['transaction_costs'] for m in metrics]))
            }
            ...
            try:
                with open(results_file, 'w') as f:
                    json_results = {
                        ...
                        'score_difference': float(validation_score - self.best_validation_metric),
    ...
    def evaluate(self, env: TradingEnv, render=False):
        ...
        try:
            obs, info = env.reset()
            # --- Assert observation structure --- 
            assert isinstance(obs, dict), "Evaluation: Observation from env.reset() must be a dict"
            assert 'market_data' in obs and 'account_state' in obs, "Evaluation: Observation missing keys"
            assert isinstance(obs['market_data'], np.ndarray), "Evaluation: Market data is not numpy array"
            assert isinstance(obs['account_state'], np.ndarray), "Evaluation: Account state is not numpy array"
            # --- End Assert ---
        except Exception as e:
        ...
        while not done:
            try:
                ...
                assert obs['market_data'].shape == (self.agent.window_size, self.agent.n_features), "Evaluation: Market data shape mismatch"
                assert obs['account_state'].shape == (2,), "Evaluation: Account state shape mismatch"
                # --- End Assert ---
                action = self.agent.select_action(obs)
                assert isinstance(action, (int, np.integer)), "Evaluation: Agent action is not an integer"
                ...
                next_obs, reward, done, _, info = env.step(action)
                # --- Assert env.step outputs --- 
                assert isinstance(next_obs, dict), "Evaluation: Next observation must be a dict"
                assert isinstance(reward, (float, np.float32, np.float64)), "Evaluation: Reward is not a float"
                assert isinstance(done, (bool, np.bool_)), "Evaluation: Done flag is not boolean"
                assert isinstance(info, dict), "Evaluation: Info object is not a dict"
                assert not np.isnan(reward) and not np.isinf(reward), "Evaluation: Reward is NaN or Inf"
                # --- End Assert --- 
                ...
                if steps % 50 == 0:
                    ...
                    logger.info(f"  Step {steps}: Portfolio=${portfolio_value:.2f}, Action={action:d}, Price=${current_price:.2f}")

                obs = next_obs
            except Exception as e:
                 logger.error(f"Error during evaluation step {steps}: {e}", exc_info=True)
        ...
        return_pct = ((final_portfolio - initial_portfolio_val) / (initial_portfolio_val + 1e-9)) * 100 # Add epsilon for safety
        ...
        portfolio_growth = (portfolio_values[-1] / (portfolio_values[0] + 1e-9)) if len(portfolio_values) > 0 else 1.0
        ...
        if len(prices) > 1:
            ...
            if price_growth > 1e-9:
                ...
                performance_str = f"OUTPERFORMS asset by {(outperform - 1) * 100:.2f}%" if outperform > 1 else f"UNDERPERFORMS asset by {(1 - outperform) * 100:.2f}%"
                logger.info(f"Strategy {performance_str}")
            else:
                 logger.info("Price growth factor is zero or negative, cannot compare performance.")
