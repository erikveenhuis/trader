...
class NoisyLinear(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.training:
            assert self.weight_epsilon.shape == (self.out_features, self.in_features), "weight_epsilon shape mismatch"
...
class PositionalEncoding(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        assert x.shape[1] <= self.pe.shape[0], f"Input sequence length ({x.shape[1]}) exceeds max_len ({self.pe.shape[0]}) of PositionalEncoding"
        ...
        assert x.shape == (x.shape[0], x.shape[1], self.d_model), "Output shape mismatch from PositionalEncoding"
        return x

# --- Start: Rainbow Network Definition ---
class RainbowNetwork(nn.Module):
    def __init__(self, config: Dict[str, Any], device: torch.device): # Accept config dict
        ...
        num_encoder_layers = config.get('transformer_layers', 1) # Default if not in config
        ...
        self.pos_encoder = PositionalEncoding(self.hidden_dim, dropout=dropout, max_len=self.window_size)
    ...
    def forward(self, market_data: torch.Tensor, account_state: torch.Tensor) -> torch.Tensor:
        ...
        assert market_data.shape[1] == self.window_size, f"Input market_data seq len ({market_data.shape[1]}) != window_size ({self.window_size})"
        assert market_data.shape[2] == self.n_features, f"Input market_data feat dim ({market_data.shape[2]}) != n_features ({self.n_features})"
        ...
        assert market_emb.shape == (batch_size, self.window_size, self.hidden_dim), "market_emb shape mismatch"
        market_emb = self.pos_encoder(market_emb)
        assert market_emb.shape == (batch_size, self.window_size, self.hidden_dim), "market_emb shape after pos_encoder mismatch"
        market_features = self.transformer_encoder(market_emb)
        assert market_features.shape == (batch_size, self.window_size, self.hidden_dim), "market_features shape mismatch"
        market_agg = market_features[:, -1, :]
        assert market_agg.shape == (batch_size, self.hidden_dim), "market_agg shape mismatch"
        account_features = self.account_processor(account_state)
        assert account_features.shape == (batch_size, self.hidden_dim // 4), "account_features shape mismatch"
        shared_features = torch.cat([market_agg, account_features], dim=1)
        expected_shared_dim = self.hidden_dim + self.hidden_dim // 4
        assert shared_features.shape == (batch_size, expected_shared_dim), "shared_features shape mismatch"
        # -----------------------------------

        # --- Dueling Streams ---
        value_logits = self.value_stream(shared_features)
        assert value_logits.shape == (batch_size, self.num_atoms), "value_logits shape mismatch"
        advantage_logits = self.advantage_stream(shared_features)
        assert advantage_logits.shape == (batch_size, self.num_actions * self.num_atoms), "advantage_logits shape mismatch"
        ...
        assert q_logits.shape == (batch_size, self.num_actions, self.num_atoms), "q_logits shape mismatch"
        # -----------------------------------------

        # --- Get Log Probabilities (for training loss) ---
        log_probs = F.log_softmax(q_logits, dim=2)
        assert log_probs.shape == (batch_size, self.num_actions, self.num_atoms), "log_probs shape mismatch"
        # Check for NaN/Inf
    ...
    def get_q_values(self, market_data: torch.Tensor, account_state: torch.Tensor) -> torch.Tensor:
        ...
        assert q_values.shape == (market_data.shape[0], self.num_actions), "Output q_values shape mismatch"
        return q_values
